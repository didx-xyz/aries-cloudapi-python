...
async def check_tr_for_verifier(
    ...
    actor = await actor_by_did(pub_did)
    # Verify actor was in TR
    if not actor:
        raise CloudApiException(f"No actor with DID {pub_did}", 404)

    # 2. Check actor has role verifier
    is_verifier = await actor_has_role(actor_id=actor.id, role="verifier")
    if not is_verifier:
    ...
    generated_did = [ed25519_verkey_to_did_key(k) for k in credential_ids]
    schemas_from_tr = (await get_trust_registry)["schemas"]
    # Verify the schemas are actually in the lsit from TR
    schemas_valid_list = [id in schemas_from_tr for id in generated_did]
