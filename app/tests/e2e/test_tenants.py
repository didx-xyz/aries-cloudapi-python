...
from app.facades import acapy_wallet, trust_registry, acapy_connections
...
@pytest.mark.asyncio
async def test_get_tenant_auth_token(
    ecosystem_admin_client: AsyncClient, ecosystem_admin_acapy_client: AcaPyClient
):
    name = uuid4().hex
    response = await ecosystem_admin_client.post(
...
async def test_update_tenant_ecosystem_verifier_to_issuer(
    ...
    async with get_tenant_controller(Role.ECOSYSTEM, acapy_token) as tenant_controller:
        ...
        connections = await acapy_connections.get_connections_by_invitation_key(
            tenant_controller, endorser_did.verkey
        )
        print((await tenant_controller.connection.get_connections()).json(indent=4))
        print(endorser_did)
    ...
    assert new_actor
    assert_that(new_actor).has_name(name)
    assert_that(new_actor).has_did(f"did:sov:{public_did}")
    assert_that(new_actor).has_roles(["issuer", "verifier"])
    ...
    assert_that(new_tenant).has_updated_at(wallet.updated_at)
    assert wallet.settings["wallet.name"].startswith("ecosystem.")
