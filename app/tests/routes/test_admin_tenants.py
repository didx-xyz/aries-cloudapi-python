...
import pytest
from aries_cloudcontroller import CreateWalletTokenRequest, CreateWalletResponse
...
async def test_create_tenant_success():
    ...
    with patch(
        ...
        response = await create_tenant(body=body, admin_auth=TENANT_ADMIN_AUTHED)
...
async def test_get_tenants_success(group_id):
    ...
    with patch(
        ...
        mock_tenant_from_wallet_record.call_count == len(wallets_return_obj.results)
