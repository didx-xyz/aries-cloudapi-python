...
When specifying a ```non_revoked``` time frame that comes before the revoked date, the proof request will pass. This is by design as the credential was valid in that time frame. Lets demonstrate:
