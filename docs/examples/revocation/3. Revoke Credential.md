...
By setting the ```"auto_publish_on_ledger"``` to *false* an issuer can all revoke on all the credential exchange ids they would like to revoke without doing multiple ledger operations.
...
Once revoked is called on all the id's that need to be revoked then the revocations can be published to the ledger.

[Publish Revocations](./4.%20Publish%20Revocations.md)
